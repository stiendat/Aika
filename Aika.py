            ...
            if split in glob.config['filters'] or any(w in after.content.lower() for w in glob.config['substring_filters']):
                ...
                try:
                    await after.author.send(
                        'Hello,\n\n'
                        'Your message has been removed as it has been deemed unsuitable.\n\n'
...
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState) -> None: # TODO: check if they left dragmein, and delete embed.. if that's even possible..
    ...
    embed = discord.Embed(
        title       = f'{member} wants to join the fun.',
...
async def on_message(message: discord.Message) -> None:
    ...
    if message.channel.id == glob.config['akatsuki_rank_request_id']:
        await message.delete()
        ...
        if b['ranked'] in (2, 5): # Map is already ranked/loved
            await message.author.send(
                ...
                f'{"ranked" if b["ranked"] == 2 else "loved"}.\n\n'
        ...
        .set_author(url  = f'https://pisstau.be/d/{b["sid"]}', name = b['song_name']) \
        ...
        embed_dm = discord.Embed(
            title       = "Your beatmap nomination request has been sent to Beatmap Nomination Team for review.",
