...
class User(commands.Cog):
    ...
    async def recent_score_command(self, ctx) -> None:
        ...
        if not username_safe: # User didn't specify a username; use their connected osu!Akatsuki account if their Discord is linked..
            ...
            if not res or not res['id']:
                await ctx.send('Please either specify a username, or connect your game account with the !linkosu command.\n\n> `!recent <username (default: linked game account)> <-rx>`')
                return
        else:
            res = glob.db.fetch('SELECT username, id FROM users WHERE username_safe = %s', [username_safe])
            if not res:
                await ctx.send(f'Sorry, but I could not find a user by that name.\nIf you believe this is a bug, please report it to dev.')
                return
        ...
        .set_author(
            ...
            name     = res['username'],
            icon_url = f'https://a.osuvnfc.xyz/{res["id"]}') \
        .add_field(
    ...
    @commands.command(
        name        = 'linkosu',
        description = 'Links your discord account to your osuvnfc account.',
        aliases     = ['linkdiscord']
    )
    async def link_osu_account(self, ctx) -> None:
        # verify_code = str(uuid.uuid4().int)
        # res = glob.db.fetch('SELECT userid, verify_id FROM discord_roles WHERE discordid = %s', [ctx.author.id])
        # if res and res['userid']:
        #     await ctx.send(f"Your account is already linked to https://osuvnfc.xyz/u/{res['userid']}.\nIf you'd like to link your Discord to another account, please contact <@{glob.config['discord_owner_userid']}>.")
        #     return
        # elif res and res['verify_id']:
        #     s = "It seems as if you've already initiated the linking process."
        # else:
        #     glob.db.execute("INSERT INTO `discord_roles` (`id`, `userid`, `discordid`, `verify_id`) VALUES (NULL, '0', '{}', '{}');".format([ctx.author.id], verify_code))
        #     s = 'Linking process initiated.'

        # await ctx.send(f'{s}\nPlease enter the following command on the osuvnfc server to `AC`, or in #osu to complete verification.\n> `!linkdiscord {verify_code}`')
        # return
        messages: List[str] = ctx.message.content.split(' ')[1:]
        if (len(messages) != 1):
            await ctx.send('Please log in osu! and use !linkdiscord in #osu or pm `AC` bot')
            return
        else:
    ...
    async def sync_osu_roles(self, ctx) -> None:
        ...
        if res['privileges'] & 7: role = discord.utils.get(ctx.message.guild.roles, name='Donor') # Supporter
        else:
            await ctx.send("I couldn't find any roles to sync!\n\nIf you have recently donated and are trying to claim your discord perks, please contact mod directly.")
            return
    ...
    async def rawfrom_command(self, ctx) -> None:
        ...
        if len(messages) > 3: # Should never have more than a username and possibly -rx flag.
            await ctx.send('Invalid syntax. Please use the following syntax:\n> `!ppfrom <username (default: linked game account)> <-rx>`')
            return
        ...
        if not username_safe: # User didn't specify a username; use their connected osu!Akatsuki account if their Discord is linked..
            res: Optional[Dict[str, Union[str, int]]] = glob.db.fetch(
                'SELECT users.id FROM discord LEFT JOIN users ON discord_roles.userid = users.id WHERE discord.discordid = %s',
                [ctx.author.id]
            )
            if not res or not res['id']:
                await ctx.send('Please either specify a username, or connect your osuvnfc account with the !linkosu command.\n\n> `!recent <username (default: linked game account)> <-rx>`')
                return
        else:
